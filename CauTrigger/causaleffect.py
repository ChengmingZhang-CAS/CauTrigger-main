...
def joint_uncond_raw(params, model, data, index, alpha_vi=False, beta_vi=True, eps=1e-6, device=None):
    """
    joint_uncond:
        Sample-based estimate of "joint, unconditional" causal effect, -I(alpha; Yhat).
    Inputs:
    ...
    q = torch.zeros(params['M'], device=device)
    feat = data[index].repeat(params['N_alpha'] * params['N_beta'], 1)
    x_w, _ = model.feature_selector(feat, keep_top=True, keep_not_top=False)
    latent = model.encoder(x_w)
    mu, std = latent["qz_m"], latent["qz_v"].sqrt()
    if alpha_vi:
        alpha_mu = mu[:, :params['K']].mean(0)
        alpha_std = std[:, :params['K']].mean(0)
    ...
    dpd, prob = model.dpd_model(x_down)
...
def beta_info_flow_raw(params, model, data, index, alpha_vi=True, beta_vi=False, eps=1e-6, device=None):
    I = 0.0
    q = torch.zeros(params['M'], device=device)
    feat = data[index].repeat(params['N_alpha'] * params['N_beta'], 1)
    x_w, _ = model.feature_selector(feat, keep_top=True, keep_not_top=False)
...
def joint_uncond_single_dim_raw(params, model, data, index, dim, alpha_vi=False, beta_vi=True, eps=1e-8, device=None):
    """
    joint_uncond_single_dim:
        Sample-based estimate of "joint, unconditional" causal effect
        for single latent factor, -I(z_i; Yhat). Note the interpretation
    ...
    q = torch.zeros(params['M'], device=device)
    feat = data[index].repeat(params['N_alpha'] * params['N_beta'], 1)
    x_w = model.feature_selector(feat, keep_top=True, keep_not_top=False)
    latent = model.encoder(x_w)
    mu, std = latent["qz_m"], latent["qz_v"].sqrt()
    if alpha_vi:
        alpha_mu = mu[:, dim].mean(0)
